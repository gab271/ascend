...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      setError(error.message?.includes('already registered') ? 'Ese correo ya tiene una cuenta. ¿Quieres iniciar sesión?' : error.message);
      setLoading(false);
    } else {
      setEmailSent(true);
      setLoading(false);
    ...
    return (
      <AuthLayout variant="register">
        <div style={{ width: '100%', maxWidth: 420 }}>
          ...
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(52px,5vw,80px)', lineHeight: 0.88, marginBottom: 24, animation: 'entry-up 0.55s ease 0.08s forwards', opacity: 0, animationFillMode: 'forwards' }}>
  ...
  return (
    <AuthLayout variant="register">
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '5px 12px 5px 10px', background: 'rgba(51,230,161,0.07)', border: '1px solid rgba(51,230,161,0.2)', borderRadius: 100, marginBottom: 28, animation: 'entry-left 0.5s ease forwards', opacity: 0, animationFillMode: 'forwards' }}>
          <span style={{ display: 'inline-block', width: 6, height: 6, borderRadius: '50%', background: '#33E6A1', boxShadow: '0 0 6px #33E6A1', animation: 'pulse-glow 2s ease-in-out infinite', flexShrink: 0 }} />
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: '#33E6A1', letterSpacing: '0.18em' }}>INICIALIZACIÓN DE NUEVO OPERADOR</span>
        </div>

        <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(56px, 6vw, 82px)', lineHeight: 0.88, letterSpacing: '0.01em', marginBottom: 16, animation: 'entry-up 0.55s ease 0.07s forwards', opacity: 0, animationFillMode: 'forwards' }}>
          <span style={{ color: '#F5F7FB' }}>ACTIVAR</span><br />
          <span style={{ color: 'transparent', WebkitTextStroke: '1.5px rgba(255,255,255,0.15)' }}>MI</span>
          {' '}
          <span style={{ background: 'linear-gradient(95deg, #33D1FF 0%, #33E6A1 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>PERFIL</span>
        </div>

        <p style={{ fontFamily: 'var(--font-body)', fontSize: 15, color: 'rgba(160,174,203,0.85)', lineHeight: 1.65, marginBottom: 8, animation: 'entry-up 0.55s ease 0.12s forwards', opacity: 0, animationFillMode: 'forwards' }}>
          Elige tu nombre de operador y configura tu acceso. Tu ascenso empieza con este paso.
        </p>

        <p style={{ fontFamily: 'var(--font-ui)', fontSize: 13, color: 'rgba(160,174,203,0.5)', marginBottom: 40, animation: 'entry-up 0.55s ease 0.14s forwards', opacity: 0, animationFillMode: 'forwards' }}>
          ¿Ya tienes cuenta?{' '}
          <Link to="/login" style={{ color: '#33D1FF', fontWeight: 700, textDecoration: 'none', borderBottom: '1px solid rgba(51,209,255,0.25)', paddingBottom: 1, transition: 'border-color 0.2s' }}
        ...
        <form onSubmit={handleSubmit}>
          <AuthInput label="Nombre de operador" type="text" value={form.username} onChange={v => setForm({ ...form, username: v })} icon={<User size={15} />} placeholder="AgenteSombra_99" focused={focused === 'username'} onFocus={() => setFocused('username')} onBlur={() => setFocused(null)} delay={0.16} />
          <AuthInput label="Correo electrónico" type="email" value={form.email} onChange={v => setForm({ ...form, email: v })} icon={<Mail size={15} />} placeholder="agente@dominio.com" focused={focused === 'email'} onFocus={() => setFocused('email')} onBlur={() => setFocused(null)} delay={0.22} />
          <AuthInput label="Contraseña" type={showPass ? 'text' : 'password'} value={form.password} onChange={v => setForm({ ...form, password: v })} icon={<Lock size={15} />} placeholder="Mín. 8 caracteres" focused={focused === 'pass'} onFocus={() => setFocused('pass')} onBlur={() => setFocused(null)} delay={0.28}
          ...
          <AuthInput
            ...
            type={showConf ? 'text' : 'password'} value={form.confirm} onChange={v => setForm({ ...form, confirm: v })} icon={<Lock size={15} />} placeholder="Repite la contraseña" focused={focused === 'confirm'} onFocus={() => setFocused('confirm')} onBlur={() => setFocused(null)} delay={0.34} error={mismatch}
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 28, animation: 'entry-up 0.5s ease 0.38s forwards', opacity: 0, animationFillMode: 'forwards' }}>
          ...
          <AuthButton loading={loading} delay={0.43} variant="cyan">
            {loading ? 'INICIALIZANDO PERFIL...' : <><span>ACTIVAR OPERADOR</span><ArrowRight size={15} style={{ flexShrink: 0 }} /></>}
          </AuthButton>

          <div style={{ display: 'flex', justifyContent: 'center', gap: 20, marginTop: 20, flexWrap: 'wrap', animation: 'entry-up 0.5s ease 0.5s forwards', opacity: 0, animationFillMode: 'forwards' }}>
