    ...
    <AuthLayout variant="register">
      <div style={{ width: '100%' }}>
        ...
        <form onSubmit={handleSubmit}>
          <AuthInput label="Nombre de usuario" type="text" value={form.username} onChange={v => setForm({ ...form, username: v })} icon={<User size={15} />} placeholder="tu_usuario" focused={focused === 'username'} onFocus={() => setFocused('username')} onBlur={() => setFocused(null)} delay={0.1} />
          <AuthInput label="Correo electrónico" type="email" value={form.email} onChange={v => setForm({ ...form, email: v })} icon={<Mail size={15} />} placeholder="tu@correo.com" focused={focused === 'email'} onFocus={() => setFocused('email')} onBlur={() => setFocused(null)} delay={0.16} />
          <AuthInput label="Contraseña" type={showPass ? 'text' : 'password'} value={form.password} onChange={v => setForm({ ...form, password: v })} icon={<Lock size={15} />} placeholder="Mín. 8 caracteres" focused={focused === 'pass'} onFocus={() => setFocused('pass')} onBlur={() => setFocused(null)} delay={0.22}
          ...
          <AuthInput
            ...
            type={showConf ? 'text' : 'password'} value={form.confirm} onChange={v => setForm({ ...form, confirm: v })} icon={<Lock size={15} />} placeholder="Repite la contraseña" focused={focused === 'confirm'} onFocus={() => setFocused('confirm')} onBlur={() => setFocused(null)} delay={0.28} error={mismatch}
          ...
          <AuthButton loading={loading} delay={0.38} variant="cyan">
            {loading ? 'Creando cuenta...' : <><span>Crear cuenta</span><ArrowRight size={15} style={{ flexShrink: 0 }} /></>}
          </AuthButton>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginTop: 18, flexWrap: 'wrap', animation: 'entry-up 0.5s ease 0.44s forwards', opacity: 0, animationFillMode: 'forwards' }}>
            {['Gratis', 'Sin tarjeta', 'Progreso visible desde día 1'].map(t => (
              <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <CheckCircle2 size={10} color="rgba(51,230,161,0.45)" />
                <span style={{ fontFamily: 'var(--font-ui)', fontSize: 11, color: 'rgba(160,174,203,0.4)', fontWeight: 500 }}>{t}</span>
              </div>
            ))}
          </div>
        </form>
